...
2. Один раз сохраните личные credentials Confluence командой `настроить confluence` (если планируете анализировать ссылки).
3. Выберите тип(ы) проекта: `BI`, `DWH`, `RPA`, `MDM`.
4. Отправьте документы:
   - прикрепленные файлы;
   - ссылки на Confluence.
5. По команде `🔄 начать анализ` бот запускает обработку.
6. Получите сообщение со сводкой и PDF-отчет.
7. Для нового цикла: `🚀 новый анализ`.

### Команды Confluence-credentials

- `настроить confluence` — первичная настройка логина и пароля (сохраняются в БД по пользователю).
- `изменить логин confluence` — смена логина без изменения пароля.
- `изменить пароль confluence` — смена пароля/токена.
- При ошибке аутентификации бот делает 1 попытку подключения и просит обновить пароль.
...
- Логин/пароль Confluence пользователей сохраняются в SQLite БД бота (персонально для каждого пользователя).

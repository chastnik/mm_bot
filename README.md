...
- **ИИ анализ**: Использование GPT/LLM для поиска артефактов в документах
- **Честный анализ**: Статус артефакта определяется только по ответу LLM, без поиска по ключевым словам
- **Отключение размышлений LLM**: В системный промпт добавлен параметр `/no_think`, модель не размышляет в тегах <think>
- **PDF отчеты**: Автоматическая генерация структурированных отчетов с шапкой "БИТ.Цифра"
